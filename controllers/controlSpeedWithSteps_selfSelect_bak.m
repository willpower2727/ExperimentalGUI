...
%                 RTOTime(RstepCount) = TimeStamp;
    ...
    if ~isnan(velR(RstepCount)) && ~isnan(velL(LstepCount))
        ...
        if (RstepCount<N-1 && ~isnan(velR(RstepCount+1))) && (LstepCount<N-1 && ~isnan(velL(LstepCount+1))) %Next step won't be self-controlled for EITHER belt.
            ...
            enableMemory=false;
            if smoothReturn && RstepCount<N-3 && LstepCount<N-3 %Smooth transition to computer control
                velR(RstepCount+[1:2])=sentR+(velR(RstepCount+3)-sentR)*[.33,.67];
                velL(LstepCount+[1:2])=sentL+(velL(LstepCount+3)-sentL)*[.33,.67];
            end
        end
    end
