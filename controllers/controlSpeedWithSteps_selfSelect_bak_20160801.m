        ...
        switch mode
            ...
            case 4 %Closed-loop mode
                if RstepCount>lastRstepCount || LstepCount>lastLstepCount %Only updating once per step
                    ...
                    newDiff=.9*lastDiff+.1*auxiliar; %With this filter, it will take ~10 steps to reach 63% of auxiliar value
                    %newDiff=auxiliar;
                    if ~isnan(newDiff) %Only doing the update if we have a numerical value
                        if abs(newDiff-lastDiff)>2 %Clipping changes to be limited to 3mm/s per step on each belt
                            newDiff=lastDiff+2*sign(newDiff-lastDiff);
                        end
