...
c = regexp(temp,' ');
temp(c) = '_';
savename = [temp '_SL_BF_FT_' profilename];
...
datlog.PS.data=zeros(300*length(velR)+7200,2);
...
datlog.targets.header = {'Step #','ANK-ANK','HIP-ANK'};
datlog.targets.Rdata = zeros(length(velR)+20,3);
datlog.targets.Ldata = zeros(length(velL)+20,3);
...
%     set(ghandle.LBeltSpeed_textbox,'String',num2str(LBS/1000));
    ...
    test = MyClient.GetDeviceOutputValue( 'pulse', 'PS' );
    datlog.PS.data(frameind.Value)= test.Value;
    ...
    if strcmp(SubjectName,'DK2')
        ...
        RGT = MyClient.GetMarkerGlobalTranslation( SubjectName, 'RGT');
        LGT = MyClient.GetMarkerGlobalTranslation( SubjectName, 'LGT');
...
%                 RHSTime(RstepCount) = TimeStamp;
                ...
                datlog.targets.Rdata(RstepCount-1,:) = [RstepCount-1,LANK-RANK,(RGT+LGT)/2-RANK];
...
%                 LHSTime(LstepCount) = TimeStamp;
                ...
                datlog.targets.Ldata(LstepCount-1,:) = [LstepCount-1,RANK-LANK,(RGT+LGT)/2-LANK];
...
%                 RTOTime(RstepCount) = TimeStamp;
    ...
    elseif (velR(RstepCount) ~= old_velR.Value) || (velL(LstepCount) ~= old_velL.Value) && LstepCount<N && RstepCount<N
        payload = getPayload(velR(RstepCount),velL(LstepCount),1000,1000,cur_incl);
        sendTreadmillPacket(payload,t);
        datlog.TreadmillCommands.sent(RstepCount,:) = [velR(RstepCount),velL(LstepCount),cur_incl,now];%record the command
...
% temp = find(datlog.TreadmillCommands.read(:,4)==0,1,'first');
...
% end
% temp = find(datlog.TreadmillCommands.sent(:,4)==0,1,'first');
...
disp(['Mean R ANK-ANK is: ' num2str(datlog.targets.Xr)]);
disp(['Mean L ANK-ANK is: ' num2str(datlog.targets.Xl)]);

datlog.targets.Ralpha = mean(datlog.targets.Rdata(:,3));
datlog.targets.Lalpha = mean(datlog.targets.Ldata(:,3));
disp(['Mean Ralpha is: ' num2str(datlog.targets.Ralpha)]);
disp(['Mean Lalpha is: ' num2str(datlog.targets.Lalpha)]);

%calculate cadence
cadenceR = 60./diff(datlog.stepdata.RHSdata(:,4));
cadenceL = 60./diff(datlog.stepdata.LHSdata(:,4));
