...
%Intiate timing
baseTime=clock; %current date and time as date vector 
curTime=baseTime;
lastCommandTime=baseTime;

%listbox = {'Function "SelfSelectedSpeed" called at: ' num2str(baseTime)};
listbox={'Function "Biofeedback" called at: 'num2str(baseTime)};
drawnow; %update the figure window with the current events 
...
RGTref=RGT.Translation;
LGTref=LGT.Translation;
RANKref=RANK.Translation;
LANKref=LANK.Translation;

% REF = mean([LPSISref(2) RPSISref(2) LASISref(2) RASISref(2)]); %this is the reference Y position!!
% YOLD = REF;

%compute mean hip position 
meanHipPos=mean([RGTref(2) LGTref(2)]);
REF=meanHipPos;
...
%     YPOS = nanmean([LPSISref(2) RPSISref(2) LASISref(2) RASISref(2)]);%the current Y position
      meanHipPos=mean([RGTref(2) LGTref(2)]);
...
%     else
if manspeed == 0 && ~isnan(YPOS) %&&=and ||=or ~=logical no
