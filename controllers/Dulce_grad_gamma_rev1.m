...
global RTOp
global RHSp
Rgammarecord = zeros(10000,1);%pre-allocate size for speed
Lgammarecord = zeros(10000,1);
LTOp = 0;
LHSp = 0;
RTOp = 0;
RHSp = 0;
...
%                 LTOTime(LstepCount) = TimeStamp;
        ...
        case 1 %single L
            if RHS
                ...
                %now calculate gamma
                Rgammarecord(RstepCount+1) = RHSp/(RHSp-RTOp);
            end
        case 2 %single R
            if LHS
                phase=4;
                ...
                drawnow;
                LHSp = LGT-LANK;
                Lgammarecord(LstepCount+1) = LHSp/(LHSp-LTOp);
            end
...
Rgammarecord(Rgammarecord == 0) = [];
Lgammarecord(Lgammarecord == 0) = [];
Rmeangamma = mean(Rgammarecord);
Lmeangamma = mean(Lgammarecord);
Rstdgamma = std(Rgammarecord);
Lstdgamma = std(Lgammarecord);
