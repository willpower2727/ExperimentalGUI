...
if nargin<8 || isempty(paramCalibFunc)
   load('lastCalibration.mat') 
    ...
    save('lastCalibration.mat','paramCalibFunc','paramComputeFunc')
end
...
fo=4000;
signS=0; %Initialize
tone=2*sin(2*pi*[1:8192]*100/4096);  %2 sec tone at 100Hz, even with noise cancelling this is heard  %2 sec tone at 100Hz, even with noise cancelling this is heard
endTone=sin(2*pi*[1:2048]*1.25*fo/4096); %.5 sec tone at 5kHz, even with noise cancelling this is heard
...
%     elapsedFrameTime=etime(curTime,lastFrameTime);
    ...
    LHS=new_stanceL && ~old_stanceL;% && (now-LTOTime(LstepCount))*72000 > .2; %At least 200ms swing time
    RHS=new_stanceR && ~old_stanceR;% && (now-RTOTime(RstepCount))*72000 > .2; %At least 200ms of swing
...
%                 RTOTime(RstepCount) = TimeStamp;
    ...
    if ~isnan(velR(RstepCount)) && ~isnan(velL(LstepCount)) %Start self-controlled mode
        sentR=velR(RstepCount);
        sentL=velL(LstepCount);
        lastL=sentL;
        lastR=sentR;
        memory=0;
        if (RstepCount<N-1 && isnan(velR(RstepCount+1))) || (LstepCount<N-1 && isnan(velL(LstepCount+1))) %Next step will be self-controlled for one of the belts
            if ~toneplayed
                sound(tone,4096) 
                toneplayed=true;
                endTonePlayed=false;
            ...
            if mode==0 && signS==0 %Determine the sign of keypresses if it hasn't been determined yet
                ...
                signS=sign(velR(RstepCount)-velL(LstepCount));
    ...
    else %At least one of the belts is already in self-controlled mode
        ...
        switch mode
            ...
            case 4 %Closed-loop mode
                if RstepCount>lastRstepCount || LstepCount>lastLstepCount %Only updating once per step
                    %Pablo changed on July 9th
                    %auxiliarL=paramCalibFunc(paramLHS);
                    %auxiliarR=paramCalibFunc(paramRHS);
                    %auxiliar=.5*(auxiliarR-auxiliarL) %Only care about difference
                    
                    midSpeed=.5*(lastR+lastL); %Mid speed of belts is fixed
                    lastDiff=.5*(lastL-lastR);
                    
                    
                    if paramLHS>0 && paramRHS>0 && paramRHS<350 && paramLHS<350 % make sure the data is not caused by mislabeling or something
                        %Add to history:
                        histL=[histL(2:end) paramLHS];
                        histR=[histR(2:end) paramRHS];
                        
                        %Do some LPF:
                        b=.1;
                        filtRHS=(1-b)*filtRHS+b*paramRHS;
                        filtLHS=(1-b)*filtLHS+b*paramLHS;
                        
                    %Deprecated on Aug 1st 2016: --------------
                        %auxiliar = .5*paramCalibFunc(paramLHS,paramRHS); %As of July 9th, paramCalibFunc is a two-input function
                        % which automatically subtracts a bias term, and is calibrated to be L-R (NOT R-L)
                    %else
                    %    auxiliar=lastDiff; %Hold constant
                    %----------------------------------
                    end
                    auxiliar=.5*paramCalibFunc(filtLHS,filtRHS);
                    
                    %diffHist=[diffHist(1:end-1) auxiliar]; %Keeping a history of N steps
                    %auxiliar=nanmedian(diffHist(end-20:end)); %Nanmedian of the last 21 steps, which represents actually 11 stride cycles
                    
                    %Changed Aug 1st:--------------
                    %newDiff=.9*lastDiff+.1*auxiliar; %With this filter, it will take ~10 steps (5 strides) to reach 63% of auxiliar value
                    newDiff=auxiliar;
                    %----------------------------
        ...
        if (RstepCount<N-1 && ~isnan(velR(RstepCount+1))) && (LstepCount<N-1 && ~isnan(velL(LstepCount+1))) %Next step won't be self-controlled for EITHER belt.
            ...
            enableMemory=false;
            M=5; %Take N strides to actually go to the desired target speed, to avoid sharp transitions 
    ...
    %every 100ms)
    elseif ((sentR ~= old_velR.Value) || (sentL ~= old_velL.Value)) && LstepCount<N && RstepCount<N && aux>.1
