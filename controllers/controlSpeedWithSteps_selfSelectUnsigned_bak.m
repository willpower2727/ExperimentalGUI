...
fo=4000;
signS=0; %Initialize
tone=sin(2*pi*[1:2048]*fo/4096); %.5 sec tone at 4kHz, even with noise cancelling this is heard
...
%                 RHSTime(RstepCount) = TimeStamp;
                ...
                plot(ghandle.profileaxes,RstepCount-1,velR(RstepCount)/1000,'o','MarkerFaceColor',[1 0.6 0.78],'MarkerEdgeColor','r');
                    ...
                    plot(ghandle.profileaxes,LstepCount-1,memoryR/1000,'o','MarkerFaceColor',[1 0.6 0.78],'MarkerEdgeColor','r');
                end
                drawnow;
...
%                 LHSTime(LstepCount) = TimeStamp;
                ...
                if ~isnan(velL(LstepCount))
                    ...
                    plot(ghandle.profileaxes,LstepCount-1,memoryL/1000,'o','MarkerFaceColor',[0.68 .92 1],'MarkerEdgeColor','b');
                end
                    drawnow;
...
%                 RTOTime(RstepCount) = TimeStamp;
    ...
    if ~isnan(velR(RstepCount)) && ~isnan(velL(LstepCount))
        ...
        if (RstepCount<N-1 && isnan(velR(RstepCount+1))) || (LstepCount<N-1 && isnan(velL(LstepCount+1))) %Next step will be self-controlled for one of the belts
            ...
            if signS==0 %Determine the sign of keypresses if it hasn't been determined yet
                firstPress=true; %Set to ignore first keypress, it is unset automatically after first keypress by AdaptationGUI.m
                signS=sign(velR(RstepCount)-velL(LstepCount)+sign(randn));
            end
            enableMemory=true;
        end
        ...
        if isnan(velR(RstepCount))
            switch mode
                case 0
                    sentR=lastR+signS*imag(memory);
                case 1
                    sentR=lastR+real(memory);
            end
        end
        if isnan(velL(LstepCount))
            switch mode
                case 0
                    sentL=lastL-signS*imag(memory);
                case 1
                    sentL=lastL-real(memory);
            end
        end
    ...
    elseif (sentR ~= old_velR.Value) || (sentL ~= old_velL.Value) && LstepCount<N && RstepCount<N
        payload = getPayload(sentR,sentL,acc,acc,cur_incl);
        sendTreadmillPacket(payload,t);
        datlog.TreadmillCommands.sent(frameind.Value,:) = [sentR,sentL,cur_incl,now];%record the command
