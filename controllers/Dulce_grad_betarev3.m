...
%Rev 3 is designed to work when the treadmill is run backwards
...
%         listbox{end+1}=log;
    ...
    Fz_L = MyClient.GetDeviceOutputValue( 'Right Treadmill', 'Fz' );
    Fz_R = MyClient.GetDeviceOutputValue( 'Left Treadmill', 'Fz' );
...
Rmeanbeta = nanmean(Rbetarecord);
Lmeanbeta = nanmean(Lbetarecord);
Rstdbeta = nanstd(Rbetarecord);
Lstdbeta = nanstd(Lbetarecord);
alphaR(alphaR==0)=[];
alphaL(alphaL==0)=[];
Rmean= nanmean(alphaR);
Lmean=nanmean(alphaL);
Rstd= nanstd(alphaR);
Lstd=nanstd(alphaL);
