function [] = controlbytime2(velL,velR,FzThreshold,profilename)
...
fig27h = figure(27);
set(fig27h,'Position',[970 100 930 900])

figure(27) %for real time plotting of SLA
subplot(3,2,1)
xlabel('Stride');
ylabel('L Step Length')
xlim([0 600]);
ylim([0 1000]);
grid on
hold on
...
LstepCount=1;
RstepCount=1;
Ozzr = 1; %counter used to oscillate which gait cycles include speed updates and which ones are used to compute SLA, added 10/21/2016
Ozzl = 1;
lastRead=now; %for reading from the treadmill
...
%             rflag = 0;
    ...
    elseif Fz_R.Value <=-30 && histRz.Value > -30 %RHS
        ...
        if rflag == 0 %plot SLA
            ...
            figure(27)
            subplot(3,2,2)
            scatter(RstepCount,RSL,'o','filled','red');
            subplot(3,2,5)
            scatter(RstepCount,SLA,'o','filled','red');
            subplot(3,2,4)
            scatter(rspeed,RSL,'o','filled','red');
            subplot(3,2,6)
            scatter(rspeed,SLA,'o','filled','red');
            drawnow
        end
        rflag = 0;
        rhs = 1;
    ...
    elseif Fz_L.Value <= -30 && histLz.Value > -30 %LHS
        ...
        if lflag == 0
           ...
           SLA = (RSL-LSL)/(RSL+LSL);
            figure(27)
            subplot(3,2,1)
            scatter(LstepCount,LSL,'o','filled','blue');
            subplot(3,2,5)
            scatter(LstepCount,SLA,'o','filled','blue');
            subplot(3,2,3)
            scatter(lspeed,LSL,'o','filled','blue');
            subplot(3,2,6)
            scatter(lspeed,SLA,'o','filled','blue');
            drawnow
        end
        lflag = 0;
        lhs = 1;
